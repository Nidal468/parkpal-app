...
import { CalendarIcon, Clock, MapPin, Star, Shield, Wifi, Camera, Car, Loader2, ExternalLink } from "lucide-react"
import { PARKPAL_SKUS } from "@/lib/commerce-layer-config"
import type { ParkingSpace } from "@/lib/supabase-types"
import { toast } from "@/hooks/use-toast"

export interface ParkpalBookingData {
...
interface ParkpalBookingModalProps {
...
export function ParkpalBookingModal({
  space,
  isOpen,
  onClose,
  selectedDates,
  selectedTime,
  defaultSku = "DAY",
}: ParkpalBookingModalProps) {
  const [bookingData, setBookingData] = useState<Partial<ParkpalBookingData>>({
    sku: defaultSku,
    quantity: 1,
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const finalBookingData: ParkpalBookingData = {
      ...
      console.log("🚗 Submitting Parkpal booking via demo-store-core...")

      // Call the demo-store integration endpoint
      const response = await fetch("/api/parkpal/demo-store-booking", {
        method: "POST",
        headers: {
      ...
      toast({
        ...
        description: `Your parking space has been reserved. Order ID: ${result.booking.demoStoreOrderId}`,
      })
              ...
              <CardContent className="space-y-4">
                ...
                <div className="p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-center gap-2 text-blue-700">
                    ...
                    <span className="text-sm font-medium">Demo Store Integration</span>
                  </div>
                  <p className="text-xs text-blue-600 mt-1">Powered by your forked demo-store-core backend</p>
                </div>
              </CardContent>
            </Card>
                  ...
                  >
                    ...
                    <SelectContent>
                      <SelectItem value="HOUR">
                        Hourly - ${PARKPAL_SKUS.HOUR.price}/hour
                        <Badge variant="outline" className="ml-2 text-xs">
                          {PARKPAL_SKUS.HOUR.id}
                        </Badge>
                      </SelectItem>
                      <SelectItem value="DAY">
                        Daily - ${PARKPAL_SKUS.DAY.price}/day
                        <Badge variant="outline" className="ml-2 text-xs">
                          {PARKPAL_SKUS.DAY.id}
                        </Badge>
                      </SelectItem>
                      <SelectItem value="MONTH">
                        Monthly - ${PARKPAL_SKUS.MONTH.price}/month
                        <Badge variant="outline" className="ml-2 text-xs">
              ...
              <div className="flex gap-2">
                ...
                >
                  {isSubmitting ? (
                    ...
                    `Book via Demo Store - $${totalPrice.toFixed(2)}`
                  )}
                </Button>
              </div>
            </form>
